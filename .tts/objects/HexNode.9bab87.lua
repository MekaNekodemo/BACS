local HEXNODE_NEIGHBOUR_LIMIT = 6
...
function initMemoTable()
    ...
    if(hexMemoTable["hasFullNeighbours"] == nil) then
        hexMemoTable["hasFullNeighbours"] = false
    end
...
function addNeighbour(neighbourNode)
    local memoTable = getMemoTable()
    local neighbours = memoTable.neighbours
    local neighbourGUID = neighbourNode.getGUID()

    if containsGUID(neighbours, neighbourGUID) then
        return
    end

    table.insert(neighbours, neighbourGUID)
    updateHasFullNeighbours(memoTable)
end

function updateHasFullNeighbours(memoTable)
    local neighbours = memoTable.neighbours

    if (#neighbours > HEXNODE_NEIGHBOUR_LIMIT) then
        error("Too many neighbours ("..#neighbours..") on node "..self.getGUID())
    end
    
    if (#neighbours < HEXNODE_NEIGHBOUR_LIMIT) then
        memoTable.hasFullNeighbours = false
    elseif (#neighbours == HEXNODE_NEIGHBOUR_LIMIT) then
...
function GUIDLabel() end
function spawn_GUIDLabel()
    local guid = self.getGUID()
    local label = tostring(guid)

    self.createButton({
        click_function = "GUIDLabel",
        function_owner = self,
        label          = label,
        position       = {0, 0.3, -5},
        rotation       = {0, 0, 0},
...
function containsGUID(list, guid)
